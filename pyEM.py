...
def mergemap(mapitem, crop=False, black=False,
    ...
    if os.path.splitext(mapfile)[1] not in mrcext:
        ...
        if mapheader['stacksize'] > 1:
            ...
            if os.path.exists(mdocname):
                ...
                if '[MontSection = 0]' not in mdoclines:  # older mdoc file format, created before SerialEM 3.7x
                    print(
                        'Warning: mrc stack without montage information.'
                        ' Assume pixel size is consistent for all sections.')
                    str1 = mdoclines[0]
            ...
            # rotation = float(mdoc_item(mdoclines,'ZValue = '+str(mapsection))['RotationAngle'][0])
                ...
                if mapheader['stacksize'] > montage_tiles:
                    print(
                        'WARNING: Multiple maps stored in an MRC stack without mdoc file for metadata.'
                        ' I will guess the pixel size.')
                ...
                tilepos1 = o1[20:-1]
                tilepos = numpy.array([numpy.fromstring(tilepos1[0], dtype=float, sep=' '),
                                       numpy.fromstring(tilepos1[1], dtype=float, sep=' ')])
        ...
        if mapheader['stacksize'] < 2:
            ...
            if blendmont:
                ...
                for j, item in enumerate(tilepx):
    ...
    if numpy.abs(tpx).max() > 0:
        xstep = tpx[tpx > 0].min()
    else:
        xstep = 1
    if numpy.abs(tpy).max() > 0:
        ystep = tpy[tpy > 0].min()
    else:
        ystep = 1
    ...
    # cropping of merged file to only include areas of interest.
    # The user needs to create an IMOD model file and close 3dmod to proceed.
    if crop & blendmont:
        if not os.path.exists(mergebase + '_crop.mrc'):
            loopcount = 0
            print(
                'waiting for crop model to be created ... '
                'Please store it under this file name: \"' + mergebase + '.mod\".')
            ...
            while not os.path.exists(mergebase + '.mod'):
                if loopcount > 20:  # wait for 6.5 minutes for the model file to be created.
                    ...
                    break
                print(
                    'waiting for crop model to be created in IMOD... '
                    'Please store it under this file name: \"' + mergebase + '.mod\".')
                time.sleep(20)
...
def call_blendmont(mapfile, mergebase, mapsection, black=False):
    ...
    callcmd = 'blendmont -v -rob 1 -imi ' + '\"' + mapfile + '\"' + ' -imo \"' + mergebase + '.mrc\" -pli \"' \
              + mapfile + '.pcs\" -roo \"' + mergebase + '.mrc\" -se ' + str(mapsection) + ' -al \"' \
              + mergebase + '.al\" -sloppy -nofft '
    # os.system(callcmd)
    if black:
...
def realign_map(item, allitems):
    ...
    if item['Type'] in [['0'], ['1']]:
        # point or polygon
        if 'DrawnID' not in item.keys():
            if 'SamePosId' not in item.keys():
...
def get_pixel(navitem, mergedmap, tile=False, outline=False):
    ...
    if 'XYinPc' in navitem:
        ...
        if (numpy.diff(tilepos, axis=0)[0].max() == 0) and (type(mergedmap['Sloppy']) == bool) and (
                mergedmap['mapheader']['stacksize'] > 1):
            print(
                'Montage created using image shift! Problems in identifying'
                ' the positions of clicked points accurately possible!')
...
def ptsonmap(mapitem, pts, nav):
    ...
    for idx, p in enumerate(pts):
        ...
        if mapitem['MapFramesXY'] == ['0', '0']:
            ...
            tilecenters = merge['tilepx'] + numpy.array(
                [merge['mapheader']['xsize'] / 2, merge['mapheader']['ysize'] / 2])
            c_out = c.copy()
...
def pts2nav(im, pts, cntrs, curr_map, targetitem, nav, sloppy=False, maps=False):
    # takes pixel coordinates on an input map, generates virtual maps at their positions,
    # creates polygons matching their shape.
    # input parameters:
    ...
    if type(pts) != list:
        if type(pts) == numpy.ndarray:
            ...
            raise Exception('Wrong input format of point coordinates.')

    if type(cntrs) != list:
        if type(cntrs) == numpy.ndarray:
            cntrs = [cntrs]
        else:
    ...
    for c in cntrs:
        ...
        print('Processing object ' + str(idx + 1) + '/' + str(ntotal) + ' (%2.0f%%)' % (
                idx * 100 / ntotal) + ' at position %5u , %5u' % (c[0], c[1]))
...
def nav_selection(allitems, sel=None, acquire=True, maps=False):
    ...
    # all maps the selected points were clicked on are also returned
    ...
    if min(im2.shape) < 200:
        ...
        if numtiles < 2:
            ...
            for tile in range(numtiles ** 2):
                ...
                idocout.append(
                    'PieceCoordinates = ' + str(tilepos0[1] * tilesz[1]) + ' ' + str(tilepos0[0] * tilesz[0]) + ' 0')
